" Bizarre Workshop - Bizarre Industries
" CATCH THE STARS.
" Vim colorscheme - works in Neovim compat mode and Vim 8+

set background=dark
hi clear
if exists("syntax_on") | syntax reset | endif
let g:colors_name = "bizarre-workshop"
if !has('gui_running') && !has('termguicolors') && &t_Co < 256
  finish
endif
if has('termguicolors') | set termguicolors | endif

function! s:H(group, fg, bg, attr)
  let cmd = 'highlight ' . a:group
  if a:fg !=# '' | let cmd .= ' guifg=' . a:fg | endif
  if a:bg !=# '' | let cmd .= ' guibg=' . a:bg | endif
  if a:attr !=# '' | let cmd .= ' gui=' . a:attr . ' cterm=' . a:attr | endif
  execute cmd
endfunction

call s:H('Normal', '#E4E4E4', '#1A1815', '')
call s:H('NormalNC', '#E4E4E4', '#1A1815', '')
call s:H('CursorLine', '', '#23211C', '')
call s:H('CursorLineNr', '#C6FF24', '#1A1815', 'bold')
call s:H('LineNr', '#9C9A91', '#1A1815', '')
call s:H('SignColumn', '', '#1A1815', '')
call s:H('Visual', '', '#3A3618', '')
call s:H('Search', '#0E0E0E', '#C6FF24', '')
call s:H('IncSearch', '#0E0E0E', '#E8FF8A', 'bold')
call s:H('CurSearch', '#0E0E0E', '#E8FF8A', 'bold')
call s:H('MatchParen', '#C6FF24', '', 'bold,underline')
call s:H('Pmenu', '#E4E2DA', '#23211C', '')
call s:H('PmenuSel', '#0E0E0E', '#C6FF24', 'bold')
call s:H('PmenuSbar', '', '#2D2A23', '')
call s:H('PmenuThumb', '', '#545040', '')
call s:H('StatusLine', '#E4E2DA', '#23211C', '')
call s:H('StatusLineNC', '#9C9A91', '#23211C', '')
call s:H('TabLine', '#9C9A91', '#23211C', '')
call s:H('TabLineFill', '', '#23211C', '')
call s:H('TabLineSel', '#0E0E0E', '#C6FF24', 'bold')
call s:H('VertSplit', '#3D3A33', '#1A1815', '')
call s:H('Folded', '#9C9A91', '#23211C', '')
call s:H('FoldColumn', '#545040', '#1A1815', '')
call s:H('SpecialKey', '#2D2A23', '', '')
call s:H('NonText', '#2D2A23', '', '')
call s:H('Whitespace', '#2D2A23', '', '')
call s:H('EndOfBuffer', '#1A1815', '#1A1815', '')
call s:H('Title', '#C6FF24', '', 'bold')
call s:H('Directory', '#7BB3FF', '', 'bold')
call s:H('ErrorMsg', '#FF6F77', '', 'bold')
call s:H('WarningMsg', '#FFB85C', '', '')
call s:H('Question', '#C6FF24', '', '')
call s:H('ModeMsg', '#E4E2DA', '', 'bold')
call s:H('MoreMsg', '#5BD06B', '', '')
call s:H('WildMenu', '#0E0E0E', '#C6FF24', '')
call s:H('Cursor', '#0E0E0E', '#C6FF24', '')
call s:H('lCursor', '#0E0E0E', '#C6FF24', '')
call s:H('Comment', '#828282', '', 'italic')
call s:H('Constant', '#9DD0FF', '', '')
call s:H('String', '#E8A33D', '', '')
call s:H('Character', '#E8A33D', '', '')
call s:H('Number', '#7BB3FF', '', '')
call s:H('Boolean', '#5B9FFF', '', '')
call s:H('Float', '#7BB3FF', '', '')
call s:H('Identifier', '#E4E4E4', '', '')
call s:H('Function', '#C6FF24', '', '')
call s:H('Statement', '#FF8FCF', '', '')
call s:H('Conditional', '#FF8FCF', '', '')
call s:H('Repeat', '#FF8FCF', '', '')
call s:H('Label', '#FF8FCF', '', '')
call s:H('Operator', '#9DEAEA', '', '')
call s:H('Keyword', '#D88AE0', '', '')
call s:H('Exception', '#FF6F77', '', '')
call s:H('PreProc', '#B989E5', '', '')
call s:H('Include', '#B989E5', '', 'italic')
call s:H('Define', '#B989E5', '', 'italic')
call s:H('Macro', '#B989E5', '', 'italic')
call s:H('Type', '#7AD9D9', '', '')
call s:H('StorageClass', '#B989E5', '', 'italic')
call s:H('Structure', '#7AD9D9', '', '')
call s:H('Typedef', '#7AD9D9', '', 'italic')
call s:H('Special', '#FF9E3D', '', '')
call s:H('SpecialChar', '#FF9E3D', '', '')
call s:H('Tag', '#FF8FCF', '', '')
call s:H('Delimiter', '#828282', '', '')
call s:H('Underlined', '#7BB3FF', '', 'underline')
call s:H('Error', '#FF6F77', '', 'bold')
call s:H('Todo', '#0E0E0E', '#C6FF24', 'bold')
call s:H('DiffAdd', '#5BD06B', '#1A1815', '')
call s:H('DiffChange', '#FFB85C', '#1A1815', '')
call s:H('DiffDelete', '#FF6F77', '#1A1815', '')
call s:H('DiffText', '#E4E2DA', '#23211C', 'bold')

" CATCH THE STARS.
