...
function run(command, args, label, optional = false) {
  ...
  if (probe.status !== 0) {
    if (!optional || strict) failures.push(`${label}: ${command} not found`);
    else {
      skips.push({ label, command });
      console.log(`skip ${label}: ${command} not installed`);
    }
    return;
  }
...
for (const file of plistFiles) {
  run('xmllint', ['--noout', file], `xml plist ${file}`);
  if (process.platform === 'darwin') run('plutil', ['-lint', file], `plist ${file}`);
}
...
const badgeFile = 'devtools/github-readme-assets/bizarre-badge.svg';
if (!fs.existsSync(path.join(root, badgeFile))) {
  fail(`xml ${badgeFile}`, 'required generated badge is missing');
} else {
  run('xmllint', ['--noout', badgeFile], `xml ${badgeFile}`);
}
...
const pythonSyntaxCheck = 'import pathlib,sys; compile(pathlib.Path(sys.argv[1]).read_bytes(), sys.argv[1], "exec")';

for (const file of files('tools/ranger', (file) => file.endsWith('.py')).map(rel)) {
  run('python3', ['-c', pythonSyntaxCheck, file], `python ${file}`);
}
...
for (const file of files('apps/slack', (file) => file.endsWith('.txt')).map(rel)) {
  ...
  const bad = text.split('\n').filter((line) => {
    const trimmed = line.trim();
    return trimmed
      && !/^# Bizarre .+$/u.test(trimmed)
      && !/^(?:#[0-9A-Fa-f]{6},){7}#[0-9A-Fa-f]{6}$/u.test(trimmed);
  });
...
if (failures.length) {
  console.error(`\nvalidation failed (${skips.length} optional checks skipped)`);
  for (const item of failures) console.error(`- ${item}`);
  process.exit(1);
}

console.log(`\nvalidation passed (${strict ? 'strict' : 'portable'} mode; ${skips.length} optional checks skipped)`);
