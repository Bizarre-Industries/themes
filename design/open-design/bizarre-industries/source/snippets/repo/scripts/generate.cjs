...
const { createGeneratedFiles } = require('./lib/generated-files.cjs');

const root = path.resolve(__dirname, '..');
const check = process.argv.includes('--check');
const generatedFiles = createGeneratedFiles({ root, check });
...
function relativeLuminance(hex) {
  const channels = rgb(hex).map((value) => value / 255).map((value) => (
...
function vscodeTheme(v) {
  ...
  return {
    ...
    colors: {
      ...
      'editor.selectionForeground': fgFor(v.sel),
      ...
      'editorLineNumber.foreground': v.fgDim,
      'editorLineNumber.activeForeground': v.accentText,
      ...
      'sideBarSectionHeader.foreground': v.fgDim,
      ...
      'list.highlightForeground': v.mode === 'light' ? v.fg : v.accentText,
      ...
      'titleBar.inactiveForeground': v.fgDim,
      ...
      'panelTitle.inactiveForeground': v.fgDim,
      ...
      'input.placeholderForeground': v.fgDim,
      ...
      'notificationLink.foreground': v.accentText,
...
function zedTheme(v) {
  ...
  return {
    ...
    style: {
      ...
      'editor.line_number': v.fgDim,
      'editor.active_line_number': v.accentText,
      ...
      text: v.fg,
      'text.muted': v.fgDim,
      'text.placeholder': v.fgDim,
      'text.disabled': v.fgGhost,
      'text.accent': v.accentText,
      icon: v.fgDim,
      ...
      syntax: {
        ...
        number: { color: s.num },
        operator: { color: s.op },
        predictive: { color: v.fgDim, font_style: 'italic' },
        preproc: { color: s.pre },
...
function sublimeScheme(v) {
  ...
  return {
    ...
    rules: [
      ...
      subRule('Markup heading', 'markup.heading', v.accentText, 'bold'),
...
return {
  ...
  accent = '${v.accent}', accent_text = '${v.accentText}', accent2 = '${v.mode === 'dark' ? v.accentSoft : v.accentText}',
...
function vimColors(v) {
  const p = {
    ...
    fg4: v.fgGhost, accent: v.accent, accentText: v.accentText,
    accent2: v.mode === 'dark' ? v.accentSoft : v.accentText, selBg: v.sel, lineHL: v.line,
    ...v.syntax,
  };
  const calls = [
    ['Normal', p.plain, p.bg, ''],
    ...
    ['CursorLine', '', p.lineHL, ''],
    ['CursorLineNr', p.accentText, p.bg, 'bold'],
    ['LineNr', p.fg2, p.bg, ''],
    ['SignColumn', '', p.bg, ''],
    ['Visual', '', p.selBg, ''],
    ...
    ['MatchParen', p.accentText, '', 'bold,underline'],
    ['Pmenu', p.fg, p.bg2, ''],
    ['PmenuSel', fgFor(p.accent), p.accent, 'bold'],
    ['PmenuSbar', '', p.bg3, ''],
    ['PmenuThumb', '', p.fg4, ''],
    ['StatusLine', p.fg, p.bg2, ''],
    ['StatusLineNC', p.fg2, p.bg2, ''],
    ['TabLine', p.fg2, p.bg2, ''],
    ['TabLineFill', '', p.bg2, ''],
    ...
    ['Title', p.accentText, '', 'bold'],
    ['Directory', p.info, '', 'bold'],
    ['ErrorMsg', p.error, '', 'bold'],
    ['WarningMsg', p.warn, '', ''],
    ['Question', p.accentText, '', ''],
    ['ModeMsg', p.fg, '', 'bold'],
    ['MoreMsg', p.ok, '', ''],
...
${[
  ...
  face('minibuffer-prompt', [':foreground accent-text', ':weight bold']),
  face('mode-line', [':background bg2', ':foreground fg', `:box (:line-width 1 :color "${v.border}")`]),
  face('mode-line-inactive', [':background bg2', ':foreground dim', `:box (:line-width 1 :color "${v.border}")`]),
  ...
  face('line-number', [':foreground dim', ':background bg']),
  face('line-number-current-line', [':foreground accent-text', ':background bg', ':weight bold']),
  face('show-paren-match', [':foreground accent-text', ':weight bold', ':underline t']),
...
function kateTheme(v) {
  ...
  return JSON.stringify({
    ...
    'editor-colors': {
      ...
      'IconBorder': v.bg2,
      'LineNumbers': v.fgDim,
      'CurrentLineNumber': v.accentText,
      'Separator': v.border,
      'MarkBookmark': s.info,
    ...
    'text-styles': {
      ...
      'RegionMarker': { 'text-color': v.accentText, 'background-color': v.bg2 },
...
${[
  ...
  widget('Brace highlight style', v.accentText, v.bg, '1'),
  ...
  widget('Line number margin', v.fgDim, v.bg),
  widget('Fold margin', v.fgDim, v.bg2),
].join('\n')}
  </GUIConfigs>
  <LexerStyles>
...
function M.apply(p)
  set({
    ...
    FloatTitle = { fg = p.accent_text, bg = p.bg2, bold = true },
    SignColumn = { bg = p.bg },
    LineNr = { fg = p.fg2 },
    CursorLineNr = { fg = p.accent_text, bold = true },
    ...
    MatchParen = { fg = p.accent_text, bold = true, underline = true },
    NonText = { fg = p.bg3 },
    Whitespace = { fg = p.bg3 },
    SpecialKey = { fg = p.bg3 },
    Conceal = { fg = p.fg2 },
    ...
    ['@markup.strikethrough'] = { fg = p.fg2, strikethrough = true },
    ...
    ['@markup.list.unchecked'] = { fg = p.fg2 },
    ...
    ['@lsp.mod.deprecated'] = { fg = p.fg2, strikethrough = true },

    DiagnosticError = { fg = p.danger },
    DiagnosticWarn = { fg = p.warn },
    DiagnosticInfo = { fg = p.info },
    DiagnosticHint = { fg = p.accent_text },
    ...
    DiagnosticVirtualTextHint = { fg = p.accent_text, bg = p.bg2 },
    ...
    TelescopeMatching = { fg = p.accent_text, bold = true },
    TelescopeSelection = { fg = p.fg, bg = p.bg3, bold = true },

    NvimTreeNormal = { fg = p.fg2, bg = p.bg2 },
    NvimTreeRootFolder = { fg = p.accent_text, bold = true },
    ...
    CmpItemAbbrMatch = { fg = p.accent_text, bold = true },
    CmpItemAbbrMatchFuzzy = { fg = p.accent2, bold = true },
    CmpItemAbbrDeprecated = { fg = p.fg2, strikethrough = true },
...
function base16Slots(v) {
  ...
  return {
    ...
    base05: v.fgDim,
    base06: v.fg,
    base07: v.ansi.brWhite,
    base08: s.error,
    base09: s.num,
    base0A: v.mode === 'dark' ? v.accentSoft : v.accentText,
    base0B: s.string,
    base0C: s.rgx,
    base0D: s.fn,
    base0E: s.decorator,
...
function base16(v) {
  const slots = Object.entries(base16Slots(v)).map(([slot, color]) => `${slot}: "${noHash(color)}"`).join('\n');
  return `scheme: "${v.label}"
author: "Bizarre Industries"
${slots}`;
}

function base16Readme() {
  return `# Bizarre Industries — Base16 / tinted-theming
...
function generateBase16() {
  ...
  out('editors/neovim-base16/README.md', base16Readme());
}
...
function jetbrains(v) {
  ...
  const colorOptions = [
    ...
    ['INDENT_GUIDE', v.bg3],
    ['SELECTED_INDENT_GUIDE', v.fgFaint],
    ['LINE_NUMBERS_COLOR', v.fgDim],
    ['LINE_NUMBER_ON_CARET_ROW_COLOR', v.accentText],
    ['VISUAL_INDENT_GUIDE', v.bg3],
    ['WHITESPACES', v.bg3],
    ['ANNOTATIONS_COLOR', v.fgDim],
...
${variants.map((v) => `    "${titleSlug(v.id)}" {
  ...
  out('terminals/tmux/bizarre.tmux.conf', tmux());
  generateTerminalBacklog();
}

function tmux() {
  const stanza = (variant) => `# -- ${variant.label} --
set -g status-style "bg=${variant.bg},fg=${variant.fg}"
set -g status-left  "#[bg=${variant.accent},fg=${fgFor(variant.accent)},bold] BZR ✦ #S #[bg=${variant.bg},fg=${variant.accentText}]"
set -g status-right "#[fg=${variant.border}]✦ #[fg=${variant.fg}]%H:%M #[fg=${variant.border}]✦ #[fg=${variant.fg}]%a %b %d "
set -g status-left-length 40
set -g status-right-length 60
set -g window-status-format         "#[fg=${variant.fgDim}] #I:#W "
set -g window-status-current-format "#[bg=${variant.accent},fg=${fgFor(variant.accent)},bold] #I:#W "
set -g window-status-separator ""
set -g pane-border-style        "fg=${variant.border}"
set -g pane-active-border-style "fg=${variant.accent}"
set -g message-style "bg=${variant.accent},fg=${fgFor(variant.accent)},bold"
set -g message-command-style "bg=${variant.border},fg=${variant.fg}"
set -g mode-style "bg=${variant.accent},fg=${fgFor(variant.accent)}"
set -g clock-mode-colour "${variant.accent}"
set -g copy-mode-match-style "bg=${variant.accent},fg=${fgFor(variant.accent)}"
set -g copy-mode-current-match-style "bg=${variant.syntax.warn},fg=${fgFor(variant.syntax.warn)}"`;
...
#
# Set @bizarre-variant before sourcing this file. Unset defaults to void.

%if #{||:#{==:#{@bizarre-variant},},#{==:#{@bizarre-variant},void}}
set -g @bizarre-variant "void"
${stanza(palette.variants.void)}
${variants.filter((variant) => variant.id !== 'void').map((variant) => `%elif #{==:#{@bizarre-variant},${variant.id}}
${stanza(variant)}`).join('\n')}
%else
display-message "Bizarre: invalid @bizarre-variant '#{@bizarre-variant}'"
%endif`;
}
...
#
# Install: preserve any existing Starship config and merge this palette, format, and module styling.
...
[git_branch]
symbol = " "
style = "bg:${v.bg3} fg:${v.mode === 'light' ? v.fg : v.accentText}"
...
up_to_date = ""
untracked = "[?$count](bg:${v.bg3} fg:${v.fgDim}) "
...
"working_copy timestamp"   = "${fgFor(v.accent)}"
...
function forklift(v) {
  ...
  return JSON.stringify({
    ...
    selectionForeground: fgFor(v.sel),
    border: v.border,
    scrollbar: v.bg4,
    ...
    tabs: { background: v.bg, tabBackground: v.bg2, tabBackgroundActive: v.bg, tabForeground: v.fgDim, tabForegroundActive: v.accentText, tabIndicator: v.accent, newTabButton: v.fgFaint },
    pathBar: { background: v.bg2, foreground: v.fg, separator: v.fgFaint, current: v.accentText },
    ...
    fileColors: {
      ...
      directoryEmpty: v.fgDim,
      executable: v.accentText,
      archive: s.warn,
      image: s.hint,
      video: s.self,
      audio: s.rgx,
      ...
      extensions: {
        ...
        go: s.type, lua: s.info, c: s.ns, cpp: s.ns, h: s.pre, md: v.accentText, txt: v.fg,
    ...
    preview: { background: v.bg, foreground: v.fg, border: v.border, headerBackground: v.bg2, metaBackground: v.bg2, metaForeground: v.fgDim, metaKey: v.fgDim, metaValue: v.fg, syntaxAccent: v.accent },
    git: { untracked: s.warn, modified: s.warn, staged: s.ok, added: s.ok, deleted: s.error, renamed: s.info, conflict: s.error, ignored: v.fgDim, branchIndicator: v.accentText },
    statusBar: { background: v.bg2, foreground: v.fgDim, accent: v.accentText, separator: v.bg3, warn: s.warn, error: s.error, ok: s.ok },
...
theme[title]=${qh(v.accentText)}
theme[hi_fg]=${qh(v.accentText)}
...
${variants.map((v) => {
  ...
  return `[delta "${titleSlug(v.id)}"]
    ...
    navigate = true
    hyperlinks = true
    commit-decoration-style = box ul ${v.accentText}
    file-style = bold ${v.accentText}
    ...
    zero-style = syntax
    line-numbers-left-style = ${v.fgDim}
    line-numbers-right-style = ${v.fgDim}
...
function ezaColors(v) {
  ...
  return [
    ...
    pair('da', v.fgDim), pair('hd', v.accent, true), pair('lp', s.rgx),
    pair('im', s.hint), pair('vi', s.self), pair('mu', s.rgx), pair('lo', s.rgx),
    pair('cr', s.warn), pair('do', s.info), pair('co', s.warn), pair('tm', v.fgDim),
    pair('cm', v.fgDim), pair('bu', s.warn), pair('sc', s.ok),
...
export FZF_DEFAULT_OPTS="\${FZF_DEFAULT_OPTS:-} \\
  ...
  --color=spinner:${s.warn},header:${v.fgDim},border:${v.border},gutter:${v.bg}"`;
}
...
[mgr]
cwd = ${style(v.accentText, null, 'bold = true')}
hovered = ${style(v.fg, v.bg3)}
preview_hovered = ${style(v.fg, v.bg3)}
find_keyword = ${style(v.accentText, v.bg2, 'bold = true')}
...
normal_alt = ${style(v.accentText, v.bg2)}
...
active = ${style(v.accentText, null, 'bold = true')}
inactive = {}
...
title = {}
value = {}
selected = ${style(v.accentText, null, 'bold = true')}
...
[tasks]
border = ${style(v.border, null)}
title = ${style(v.accentText, null, 'bold = true')}
hovered = ${style(v.fg, v.bg3)}

[which]
mask = ${style(v.bg4, null)}
cand = ${style(v.accentText, null)}
rest = ${style(v.fgDim, null)}
desc = ${style(s.info, null)}
separator = " > "
separator_style = ${style(v.border, null)}

[help]
on = ${style(v.accentText, null, 'bold = true')}
...
[styles.cpu]
all_entry_color = "${v.accentText}"
avg_entry_color = "${s.info}"
cpu_core_colors = ["${v.accentText}", "${s.info}", "${s.rgx}", "${s.warn}", "${s.hint}", "${s.ok}"]

[styles.memory]
ram_color = "${v.accentText}"
cache_color = "${s.info}"
swap_color = "${s.warn}"
arc_color = "${s.rgx}"
...
[styles.tables]
headers = { color = "${v.accentText}", bg_color = "${v.bg2}", bold = true }
...
widget_title = { color = "${v.accentText}", bg_color = "${v.bg}" }
...
k9s:
  ...
  info:
    fgColor: ${yamlString(v.fg)}
    sectionColor: ${yamlString(v.accentText)}
  frame:
    border:
    ...
    menu:
      ...
      keyColor: ${yamlString(v.accentText)}
    ...
    status:
      ...
      highlightColor: ${yamlString(v.accentText)}
      killColor: ${yamlString(s.error)}
      completedColor: ${yamlString(v.fgDim)}
    title:
      fgColor: ${yamlString(v.accentText)}
      bgColor: ${yamlString(v.bg)}
      highlightColor: ${yamlString(v.accentText)}
...
function cssVars(v, selector = ':root') {
  ...
  const vars = {
    ...
    '--bizarre-accent-text': v.accentText,
...
body {
  ...
  --text-accent: var(--bizarre-accent-text);
...
html[data-theme=light] {
  ...
  --ls-link-text-hover-color: var(--bizarre-accent-text);
...
function slackThemeLine(v) {
  const colors = [v.bg2, v.bg, v.accent, fgFor(v.accent), v.bg3, v.fg, v.syntax.ok, v.syntax.error];
  return `# ${v.label}\n${colors.join(',')}`;
}
...
.theme-light {
  ...
  --interactive-active: var(--bizarre-accent-text);
...
function telegramTheme(v) {
  const pairs = {
    ...
    windowFgOver: v.fg,
    windowBoldFg: v.accentText,
    windowBoldFgOver: v.accentText,
...
c.colors.completion.category.fg = '${v.accentText}'
...
button,
[role="button"] {
  color: var(--bizarre-accent-text) !important;
}
input,
textarea,
select {
  ...
  const data = {
    ...
    variants: Object.fromEntries(variants.map((v) => [v.id, {
      ...
      colors: {
        ...
        accentText: { value: v.accentText, type: 'color' },
...
set $bizarre_accent_fill ${v.accent}
...
bar {
  colors {
    ...
    focused_workspace $bizarre_accent_fill $bizarre_accent_fill ${fgFor(v.accent)}
...
export BIZARRE_ACCENT_TEXT="${v.accentText}"
...
sketchybar --set front_app label.color="$BIZARRE_ACCENT_TEXT"`;
}
...
For Slack's legacy-theme importer, paste only the raw eight-color comma-separated line beneath the desired \`# Bizarre …\` label in \`apps/slack/bizarre-sidebar-themes.txt\`.
`);
}
...
function Bizarre-Banner {
  ...
  $gray = "$e[38;2;${rgb(v.fgDim).join(';')}m"
...
${syntaxRoles.map(([cls, key]) => `.code[data-theme="${mode}"] .tok-${cls} { color: ${s[key]};${styleFor(cls)} }`).join('\n')}`;
  ...
  const limeRoles = (mode, color) => `.code[data-theme="${mode}"][data-lime-role="functions"] .tok-f,
.code[data-theme="${mode}"][data-lime-role="functions"] .tok-method { color: ${color}; }
.code[data-theme="${mode}"][data-lime-role="keywords"] .tok-kw-ctrl,
.code[data-theme="${mode}"][data-lime-role="keywords"] .tok-kw-decl { color: ${color}; }
.code[data-theme="${mode}"][data-lime-role="strings"] .tok-s,
.code[data-theme="${mode}"][data-lime-role="strings"] .tok-tmpl { color: ${color}; }
.code[data-theme="${mode}"][data-lime-role="comments"] .tok-c { color: ${color}; }
.code[data-theme="${mode}"][data-lime-role="types"] .tok-t,
.code[data-theme="${mode}"][data-lime-role="types"] .tok-tprim { color: ${color}; }`;
  return `/* Bizarre Industries - syntax token colors. Generated from palette.js. */
${sections}

${limeRoles('dark', palette.brand.signalLime)}

${limeRoles('light', palette.brand.limeText)}`;
}
...
:root {
  ...
  --bzr-lime-text: ${palette.brand.limeText};
...
function showcaseCss() {
  ...
  --void-6: ${palette.variants.void.fgDim};
  --display-muted: ${palette.variants.void.fgFaint};
  ...
  --lime-text: ${palette.brand.limeText};
...
.h1 .gray { color: var(--display-muted); }
...
.lime-meta, .ansi-half-label, .legend-key { font-family: var(--label); font-size: 10px; color: var(--surface-muted, var(--void-6)); letter-spacing: 0.12em; text-transform: uppercase; }
...
.ansi-cell .hex { opacity: 1; font-family: var(--mono); }
.legend-grid { display: grid; grid-template-columns: repeat(6, minmax(0, 1fr)); gap: 10px; }
.legend-card { --surface-muted: ${palette.variants.void.fgDim}; border: 1px solid var(--hairline); border-radius: 6px; padding: 12px; min-height: 92px; }
.legend-dark { background: var(--void-2); }
.legend-light { --surface-muted: ${palette.variants.paper.fgDim}; background: var(--paper); color: var(--ink); border-color: var(--hairline-light); }
...
.big-type .out { color: var(--display-muted); }
...
.dot.sample { background: var(--lime); }
.tab-row { display: flex; gap: 4px; min-width: 0; }
.tab { padding: 5px 10px; border-radius: 3px; color: var(--dim, var(--void-6)); white-space: nowrap; }
...
.pane-meta { margin-left: auto; color: var(--dim, var(--void-6)); font-size: 10px; white-space: nowrap; }
...
.code .gutter { color: var(--dim, var(--void-6)); text-align: right; user-select: none; font-size: 11px; padding-top: 1px; }
.code .line.cur { background: rgba(198,255,36,.05); }
.code-footer { border-top: 1px solid var(--hairline); padding: 8px 14px; display: flex; gap: 18px; color: var(--dim, var(--void-6)); font-size: 10px; background: rgba(0,0,0,.2); }
...
.config-title strong { display: block; color: var(--accent-text); font-family: var(--display); font-size: 17px; line-height: 1.1; font-weight: 650; letter-spacing: 0; text-transform: none; }
.config-title code { display: block; margin-top: 8px; color: var(--dim); opacity: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-family: var(--mono); font-size: 10px; letter-spacing: 0; text-transform: none; }
...
.vscode-side .active { color: var(--accent-text); font-weight: 700; }
...
.shell-body .brand { color: var(--accent-text); font-weight: 700; }
...
.tool-body .value { color: var(--accent-text); font-weight: 700; }
...
window.BzrEditor = function Editor({ sample, variant, limeRole }) {
  return (
    <div className={\`pane \${variant.mode === 'light' ? 'light' : ''}\`} style={{ '--bg': variant.bg, '--dim': variant.fgDim, background: variant.bg }}>
      <div className="pane-chrome">
        <div className="dots"><span className="dot sample"></span><span className="dot"></span><span className="dot"></span></div>
      ...
      <div className="code-footer">
        <span>● preview · {variant.label}</span>
...
window.BzrStarshipTerminal = function StarshipTerminal({ variant }) {
  ...
  const Prompt = ({ branch = 'main', git = '', cmd, dur, dir = '~/projects/bench', warn = false, ok = false, runtime }) => (
    <_BzrFrag>
      <div style={styles.pwrap}>
        ...
        {branch && <span style={{ ...styles.chunk, background: smokeBg, color: branchFg, fontWeight: 500 }}>&nbsp; {branch}{' '}</span>}
  ...
  return (
    <div className={\`pane \${variant.mode === 'light' ? 'light' : ''}\`} style={{ '--bg': variant.bg, '--dim': variant.fgDim }}>
      <div className="pane-chrome">
      ...
      <div className="term" style={styles.pane}>
        ...
        <div style={styles.row}><span style={{ color: cyan }}>{'  "sample"'}</span><span style={{ color: dim }}>: </span><span style={{ color: green, fontWeight: 600 }}>true</span></div>
        <div style={{ ...styles.row, color: dim }}>{'}'}</div>
        <Prompt branch="theme/syntax" git="+5" ok cmd="npm test" dur="2.4s" runtime="node 22" />
        <div style={{ ...styles.row, color: green, fontWeight: 600 }}>sample command: npm test</div>
        <div style={{ ...styles.row, color: dim }}>sample coverage: json, toml, plist, xml, shell, lua</div>
        <div style={styles.pwrap}>
          <span style={{ ...styles.chunk, background: limeBg, color: limeFg }}>&nbsp;bzr&nbsp;</span>
          <span style={styles.wedgeR(limeBg, voidBg)}></span>
          <span style={{ ...styles.chunk, background: voidBg, color: dirFg, fontWeight: 500 }}>&nbsp;~/themes/bizarre&nbsp;</span>
          <span style={styles.wedgeR(voidBg, smokeBg)}></span>
          <span style={{ ...styles.chunk, background: smokeBg, color: branchFg, fontWeight: 500 }}>&nbsp; theme/regen </span>
...
function showcaseMain() {
  return `const P = window.BZR_PALETTE;
...
const TERMINAL_TARGETS = [
  ...
  { shot: 'terminal-kitty', name: 'Kitty', file: 'terminals/kitty/bizarre-void-hicontrast.conf', variant: 'void-hicontrast', cmd: 'kitty +kitten themes Bizarre Void Hi-Contrast' },
  ...
  { shot: 'terminal-wezterm', name: 'WezTerm', file: 'terminals/wezterm/bizarre.lua', variant: 'void', cmd: 'color_scheme = Bizarre Void' },
  ...
  { shot: 'terminal-tmux', name: 'tmux', file: 'terminals/tmux/bizarre.tmux.conf', variant: 'void', cmd: 'set -g @bizarre-variant void' },
  { shot: 'terminal-zellij', name: 'Zellij', file: 'terminals/zellij/bizarre.kdl', variant: 'void', cmd: 'theme "bizarre-void"' },
];
const TERMINAL_BACKLOG_TARGETS = [
  ...
  { shot: 'terminal-konsole', name: 'Konsole', file: 'terminals/konsole/bizarre-void-hicontrast.colorscheme', variant: 'void-hicontrast', cmd: 'ColorScheme=Bizarre Void Hi-Contrast' },
  ...
  { shot: 'terminal-xfce', name: 'XFCE Terminal', file: 'terminals/xfce-terminal/bizarre-void-hicontrast.theme', variant: 'void-hicontrast', cmd: 'ColorPalette' },
  { shot: 'terminal-gnome', name: 'GNOME Terminal', file: 'terminals/gnome-terminal/bizarre.sh', variant: 'void', cmd: './bizarre.sh bizarre-void' },
...
const EDITOR_TARGETS = [
  ...
  { shot: 'editor-jetbrains', name: 'JetBrains', file: 'editors/jetbrains/bizarre-void-hicontrast.icls', variant: 'void-hicontrast', lang: 'Kotlin' },
...
const EDITOR_BACKLOG_TARGETS = [
  ...
  { shot: 'editor-helix', name: 'Helix', file: 'editors/helix/bizarre-void-hicontrast.toml', variant: 'void-hicontrast', lang: 'TOML' },
  ...
  { shot: 'editor-visual-studio', name: 'Visual Studio', file: 'editors/visual-studio/bizarre-void-hicontrast.vstheme', variant: 'void-hicontrast', lang: 'XML' },
  { shot: 'editor-cursor', name: 'Cursor', file: 'editors/cursor/README.md', variant: 'void', lang: 'VS Code reuse' },
  { shot: 'editor-android-studio', name: 'Android Studio', file: 'editors/android-studio/README.md', variant: 'void', lang: 'JetBrains reuse' },
];
const SHELL_TARGETS = [
  { shot: 'shell-bash', name: 'Bash', file: 'shells/banner/bizarre.bash', variant: 'void', cmd: 'source shells/banner/bizarre.bash' },
  { shot: 'shell-zsh', name: 'Zsh', file: 'shells/banner/bizarre.zsh', variant: 'void', cmd: 'source shells/banner/bizarre.zsh' },
  { shot: 'shell-fish', name: 'Fish', file: 'shells/banner/bizarre.fish', variant: 'void', cmd: 'source shells/banner/bizarre.fish' },
  { shot: 'shell-powershell', name: 'PowerShell', file: 'shells/banner/bizarre.ps1', variant: 'void', cmd: '. ./shells/banner/bizarre.ps1' },
  { shot: 'prompt-starship', name: 'Starship', file: 'prompt/starship.toml', variant: 'void', cmd: 'starship init zsh' },
];
const TOOL_TARGETS = [
  ...
  { shot: 'tool-forklift', name: 'ForkLift', file: 'tools/forklift/Bizarre.json', variant: 'void', key: 'selection', value: P.variants.void.sel },
  { shot: 'tool-jujutsu', name: 'Jujutsu', file: 'tools/jujutsu/config.toml', variant: 'void', key: 'working-copy change id', value: P.brand.signalLime },
  { shot: 'tool-starship', name: 'Starship', file: 'prompt/starship.toml', variant: 'void', key: 'prompt block', value: P.brand.signalLime },
];
const CLI_TARGETS = [
  { name: 'bat', file: 'tools/bat/themes/bizarre-void.tmTheme', variant: 'void', key: 'syntax', value: 'TextMate' },
  { name: 'btop', file: 'tools/btop/bizarre-void-hicontrast.theme', variant: 'void-hicontrast', key: 'graphs', value: 'theme[]' },
  { name: 'delta', file: 'tools/delta/bizarre.gitconfig', variant: 'void', key: 'diff', value: 'feature' },
  { name: 'dircolors', file: 'tools/dircolors/bizarre.dircolors', variant: 'void', key: 'LS_COLORS', value: 'ansi256' },
  { name: 'fzf', file: 'tools/fzf/bizarre.sh', variant: 'void', key: 'picker', value: '24-bit' },
  { name: 'lazygit', file: 'tools/lazygit/config.yml', variant: 'void', key: 'panes', value: 'yaml' },
  { name: 'yazi', file: 'tools/yazi/flavors/bizarre-void-hicontrast.yazi/flavor.toml', variant: 'void-hicontrast', key: 'flavor', value: 'toml' },
  { name: 'eza', file: 'tools/eza/bizarre.sh', variant: 'void', key: 'files', value: 'EZA_COLORS' },
  { name: 'atuin', file: 'tools/atuin/themes/bizarre-paper.toml', variant: 'paper', key: 'history', value: 'Meanings' },
  { name: 'bottom', file: 'tools/bottom/bizarre-bone.toml', variant: 'bone', key: 'monitor', value: 'styles' },
  { name: 'k9s', file: 'tools/k9s/skins/bizarre-void.yaml', variant: 'void', key: 'cluster', value: 'skin' },
  { name: 'ranger', file: 'tools/ranger/colorschemes/bizarre_void_hicontrast.py', variant: 'void-hicontrast', key: 'files', value: 'python' },
  { name: 'vivid', file: 'tools/vivid/themes/bizarre-workshop.yml', variant: 'workshop', key: 'ls', value: 'yaml' },
];
const APP_TARGETS = [
  { name: 'Raycast', file: 'apps/raycast/bizarre-void.json', variant: 'void', key: 'format', value: 'json' },
  { name: 'Alfred', file: 'apps/alfred/bizarre-void-hicontrast.alfredappearance', variant: 'void-hicontrast', key: 'format', value: 'appearance' },
  ...
  { name: 'Slack', file: 'apps/slack/bizarre-sidebar-themes.txt', variant: 'void', key: 'format', value: 'sidebar' },
  { name: 'Discord', file: 'apps/discord/betterdiscord/bizarre-void.theme.css', variant: 'void', key: 'adapter', value: 'BetterDiscord' },
  { name: 'Telegram', file: 'apps/telegram/bizarre-void-hicontrast.tdesktop-theme', variant: 'void-hicontrast', key: 'format', value: 'tdesktop' },
  { name: 'Spotify', file: 'apps/spotify/spicetify/color.ini', variant: 'void', key: 'adapter', value: 'Spicetify' },
  { name: 'qutebrowser', file: 'apps/qutebrowser/bizarre-paper.py', variant: 'paper', key: 'format', value: 'python' },
];
const WEB_TARGETS = [
  { name: 'Firefox', file: 'web/firefox/bizarre-void/manifest.json', variant: 'void', key: 'format', value: 'manifest' },
  { name: 'Chrome', file: 'web/chrome/bizarre-void-hicontrast/manifest.json', variant: 'void-hicontrast', key: 'format', value: 'manifest' },
  ...
  { name: 'Docs sites', file: 'web/documentation-sites/bizarre-void-hicontrast.css', variant: 'void-hicontrast', key: 'format', value: 'css' },
];
const DESIGN_DOCS_TARGETS = [
  { name: 'Figma', file: 'design/figma/bizarre.tokens.json', variant: 'void', key: 'format', value: 'tokens' },
  { name: 'Sketch', file: 'design/sketch/bizarre.sketchpalette', variant: 'void', key: 'format', value: 'palette' },
  { name: 'Insomnia', file: 'devtools/insomnia/bizarre.js', variant: 'void', key: 'format', value: 'js' },
  { name: 'Postman', file: 'devtools/postman/bizarre-paper.css', variant: 'paper', key: 'adapter', value: 'userstyle' },
  { name: 'HTTPie', file: 'devtools/httpie/bizarre.py', variant: 'void', key: 'format', value: 'pygments' },
  { name: 'TablePlus', file: 'devtools/tableplus/bizarre-void.json', variant: 'void', key: 'format', value: 'json' },
  { name: 'DBeaver', file: 'devtools/dbeaver/bizarre-void-hicontrast.epf', variant: 'void-hicontrast', key: 'format', value: 'epf' },
  { name: 'GitHub assets', file: 'devtools/github-readme-assets/bizarre-badge.svg', variant: 'void', key: 'format', value: 'svg' },
  { name: 'MkDocs', file: 'docs-sites/mkdocs/bizarre-paper.css', variant: 'paper', key: 'format', value: 'css' },
  { name: 'Docusaurus', file: 'docs-sites/docusaurus/bizarre-bone.css', variant: 'bone', key: 'format', value: 'css' },
  { name: 'Sphinx', file: 'docs-sites/sphinx/bizarre/theme.toml', variant: 'void', key: 'format', value: 'theme.toml' },
  { name: 'LaTeX', file: 'docs-sites/latex/bizarre.sty', variant: 'void', key: 'format', value: 'sty' },
  { name: 'Typst', file: 'docs-sites/typst/bizarre.typ', variant: 'void', key: 'format', value: 'typ' },
  { name: 'Beamer', file: 'docs-sites/beamer/beamerthemebizarre.sty', variant: 'void', key: 'format', value: 'sty' },
  { name: 'reveal.js', file: 'docs-sites/reveal.js/bizarre-bone.css', variant: 'bone', key: 'format', value: 'css' },
];
const WM_TARGETS = [
  { name: 'Hyprland', file: 'wm/hyprland/bizarre-void.conf', variant: 'void', key: 'format', value: 'hyprlang' },
  { name: 'Sway', file: 'wm/sway/bizarre-void-hicontrast.conf', variant: 'void-hicontrast', key: 'format', value: 'config' },
  ...
  { name: 'yabai', file: 'wm/yabai/bizarre.sh', variant: 'void', key: 'format', value: 'shell' },
...
function cardStyle(v) {
  return { '--bg': v.bg, '--fgc': v.fg, '--dim': v.fgDim, '--accent': v.accent, '--accent-text': v.accentText, '--info': v.syntax.info, background: v.bg, color: v.fg };
}
...
function TerminalConfigCard({ target }) {
  ...
  return (
    <div className={'config-card ' + (v.mode === 'light' ? 'light' : '')} data-config-shot={target.shot} style={cardStyle(v)}>
      ...
      <div className="config-prompt"><span className="path">~/themes</span> <span className="cmd">✦</span> {target.cmd}<br/><span style={{ color: v.fgDim }}>ansi 16 · accent {v.accent}</span></div>
    </div>
  );
}
...
function MiniVscodeCode({ variant }) {
  ...
  return (
    <div className="mini-code">
      <div><span style={{ color: s.kwDecl }}>export const</span> <span style={{ color: variant.accentText }}>palette</span> <span style={{ color: s.op }}>=</span> <span style={{ color: s.punct }}>{'{'}</span></div>
...
window.BzrShowcase = function Showcase() {
  ...
  const shown = [P.variants.void, P.variants.paper, P.variants['void-hicontrast'], P.variants.workshop, P.variants.bone]
    .map((v, i) => ({ id: ['void','paper','void-hicontrast','workshop','bone'][i], ...v }));
    ...
    <div className="showcase">
      <header className="hero" data-shot="hero">
        <div className="eyebrow">
          ...
          <span className="pill">MAY 2026 SNAPSHOT</span>
          <span className="pill">EDITOR + TERMINAL SYSTEM</span>
        </div>
        <div className="hero-grid">
          <div>
            <h1 className="h1">
              <span className="stack"><span className="lime">YOUR</span> EDITOR.</span>
              <span className="stack">YOUR <span className="gray">TERMINAL.</span></span>
          ...
          <div>
            ...
            <dl className="hero-meta">
              ...
              <div><dt>paper accents</dt><dd>Lime Ink · {P.brand.limeInk} / Lime Text · {P.brand.limeText}</dd></div>
      ...
      <section className="section" data-shot="palette-ansi">
        ...
        <div className="lime-pair">
          <div className="lime-card" style={{ '--surface-muted': P.variants.void.fgDim, background: P.variants.void.bg, color: P.variants.void.fg }}>
          ...
          <div className="lime-card" style={{ '--surface-muted': P.variants.paper.fgDim, background: P.variants.paper.bg, color: P.variants.paper.fg, borderColor: 'rgba(14,14,14,.12)' }}>
            <div className="lime-swatch" style={{ background: P.brand.limeInk }}></div>
            <div><div className="lime-meta">LIGHT · FILL + TEXT</div><div className="lime-name" style={{ color: P.brand.limeText }}>Lime Ink / Lime Text</div><div className="lime-hex">{P.brand.limeInk} / {P.brand.limeText}</div><div className="lime-desc">Lime Ink carries fills and graphics; Lime Text keeps foreground accents readable on paper and bone.</div></div>
          </div>
        </div>
      ...
      <section className="section" data-shot="variants">
        ...
        <div className="variant-grid">
          {VARIANTS.map((v) => <div className="variant-card" key={v.id} style={{ '--bg': v.bg, '--fgc': v.fg, '--dim': v.fgDim }}><div className="variant-swatch"><div className="variant-name" style={{ color: v.accentText }}>{v.label}</div><div className="variant-sub">{v.sub}</div></div><div className="variant-strip">{[v.bg, v.bg2, v.bg3, v.fg, v.accent, v.syntax.info].map((hex) => <span key={hex} style={{ background: hex }}></span>)}</div></div>)}
        </div>
      </section>

      <section className="section" data-shot="terminal-colors">
        <div className="section-head"><span className="section-num">§ 05 / TERMINAL COLORS</span><h2 className="section-title">Core terminal configs use the palette.</h2><span className="section-sub">alacritty · kitty · ghostty · iterm2 · wezterm · windows terminal · tmux · zellij</span></div>
      ...
      <section className="section" data-shot="terminal-backlog">
        <div className="section-head"><span className="section-num">§ 05.B / TERMINALS</span><h2 className="section-title">Additional shipped terminal ports.</h2><span className="section-sub">foot · konsole · rio · hyper · terminator · tilix · xfce terminal · gnome terminal · black box</span></div>
        <div className="config-grid">
      ...
      <section className="section" data-shot="editor-backlog">
        <div className="section-head"><span className="section-num">§ 07.B / EDITORS</span><h2 className="section-title">Additional shipped editor ports.</h2><span className="section-sub">emacs · helix · lapce · kate · notepad++ · nova · cursor · visual studio · android studio</span></div>
        <div className="config-grid editor-grid">
      ...
      <section className="section" data-shot="cli-tui">
        <div className="section-head"><span className="section-num">§ 10 / CLI + TUI</span><h2 className="section-title">Shipped CLI and TUI ports.</h2><span className="section-sub">bat · btop · delta · dircolors · fzf · lazygit · yazi · eza · atuin · bottom · k9s · ranger · vivid</span></div>
      ...
      <section className="section" data-shot="shell-banner">
        ...
        <div className="banner-pane">
          ...
          <div>&nbsp;</div>
          <div className="quote">  The hands knew it before the plan did.</div>
          <div>&nbsp;</div>
...
function showcaseIndex() {
  ...
  const textOn = {};
  for (const v of variants) for (const hex of [v.bg, v.bg2, v.bg3, v.bg4, v.fg, v.accent, v.accentText, v.accentSoft, ...colorList(v)]) textOn[hex] = chipFgFor(hex);
...
<link rel="stylesheet" href="../node_modules/@fontsource/monaspace-neon/400.css" />
<link rel="stylesheet" href="../node_modules/@fontsource/monaspace-neon/400-italic.css" />
<link rel="stylesheet" href="../node_modules/@fontsource/monaspace-neon/600.css" />
<link rel="stylesheet" href="../node_modules/@fontsource/monaspace-neon/700.css" />
<link rel="stylesheet" href="../node_modules/@fontsource/monaspace-xenon/600.css" />
<link rel="stylesheet" href="../node_modules/@fontsource/monaspace-krypton/500.css" />
<link rel="stylesheet" href="../node_modules/@fontsource/monaspace-argon/400.css" />
...
function showcaseBundle() {
  const source = `${showcasePanes()}\n\n${showcaseTerminal()}\n\n${showcaseMain()}\n\nfunction App() {
  return <window.BzrShowcase />;
}
...
function generateShowcase() {
  ...
  out('showcase/showcase.js', showcaseBundle());
...
| Lime Ink | \`${palette.brand.limeInk}\` | fills, cursors, borders, and graphics on light surfaces |
| Lime Text | \`${palette.brand.limeText}\` | WCAG AA foreground accent on light primary and secondary surfaces |
...
|---|---|---:|---:|---:|---:|---:|
${variants.map((v) => `| ${v.label} | ${v.mode} | \`${v.bg}\` | \`${v.fg}\` | \`${v.border}\` | \`${v.accent}\` | \`${v.accentText}\` |`).join('\n')}
...
const dirty = generatedFiles.finish();
