...
#  CATCH THE STARS.
#
#  Usage:  source ~/dotfiles/bizarre/shells/banner/bizarre.fish
#  Toggle: set -gx BIZARRE_BANNER 1 (force) | 0 (off) | unset (daily)
# ─────────────────────────────────────────────────────────────
...
function _bzr_should_show \
    --argument-names stamp \
    --inherit-variable BIZARRE_BANNER
  if test "$BIZARRE_BANNER" = "0"
    return 1
  end
  if test "$BIZARRE_BANNER" = "1"
    return 0
  end
  set -l today (date +%Y-%m-%d)
  if test -r "$stamp"; and test (cat "$stamp") = "$today"
    return 1
  end
...
function bizarre_banner \
    --argument-names manifesto
  set -l lime (printf '\e[38;2;198;255;36m')
  set -l glow (printf '\e[38;2;232;255;138m')
  set -l gray (printf '\e[38;2;120;120;120m')
  ...
  printf '  %s%sBZR / SHELL / V0.1 / %s%s   %s%s✦%s   %shost: %s%s\n' \
