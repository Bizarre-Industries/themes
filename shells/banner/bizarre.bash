...
#  CATCH THE STARS.
#
#  Usage:  source ~/dotfiles/bizarre/shells/banner/bizarre.bash
#  Toggle: BIZARRE_BANNER=1 (force) | =0 (off) | unset (daily)
# ─────────────────────────────────────────────────────────────
...
_bzr_should_show() {
  ...
  local today
  today="$(date +%Y-%m-%d)"
  if [[ -r "$_BZR_STAMP" ]] && [[ "$(cat "$_BZR_STAMP")" == "$today" ]]; then
    return 1
  fi
...
bizarre_banner() {
  ...
  if [[ -r "$_BZR_MANIFESTO" ]]; then
    ...
    if (( count > 0 )); then
      ...
      line=$(sed -n "${idx}p" "$_BZR_MANIFESTO")
      printf '\n  %s%s%s%s\n' "$fg" "$b" "$line" "$r"
    fi
  fi
